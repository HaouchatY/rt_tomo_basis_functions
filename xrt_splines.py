...
class RayXRT(pxa.LinOp):
    ...
    def __init__( 
        ...
        self._fwd, self._bwd = _build_xrt(self._ndi, weighted, self._deg)
        if len(arg_shape) == 2:
            self._dr = dict(
...
def _build_xrt(ndi: pxd.NDArrayInfo, weighted: bool, deg :int):
    ...
    def ray_step(r: Ray3f) -> Ray3f:
        ...
        #r.d[1] = 1                                     
        #r.d[0] = dr.sin(0.1*r.o[0])        
        #r.d = dr.normalize(r.d) 
                 
        '''x_target = dr.floor(r.o[0]+1)
        y_target = (x_target - 200.)   # dr.sqrt(-(x_target-150)**2 + 50**2)
        r.d[1] = -y_target + r.o[1] #(y_target - r.o[1]) # r.o[1] - 100 # 8*r.o[1]#y_target - r.o[0] #1            #x_target - r.o[0] 
        r.d[0] = x_target - r.o[0] # -r.o[1] - 100          #(y_target - r.o[1]) 
        r.d[0] *= -1
        r.d = dr.normalize(r.d)'''
    ...
    def xrt_apply(
        ...
        while loop(active):
            ...
            idx_I = dr.floor(center_line)
            mask = active & dr.all(0 <= idx_I) & dr.all(idx_I < N)
            weight = dr.gather(type(I), I, flat_index(idx_I), mask)

            # Compute constants
            slope = (r_next.o - r.o)[1] /(r_next.o - r.o)[0]
            theta = dr.atan(slope)
            theta = dr.select(dr.abs(theta + dr.pi/2) < 1e-10, dr.pi/2, theta) # can be removed if corrected, but works temporarily by symmetry
            
            
            evaluate_spline = radon_transform_zwart_powell_adjusted(theta, deg)

            # Compute y : shortest distance between center of the pixel and the ray
            ...
            ######## SMALL COND #########
            small_cond = dr.select(dr.abs(theta) <= dr.pi/4, 1., 0.)
            # Compute for "Left" neighbor
            
            idx_I_l = dr.floor(0.5 * (r_next.o + r.o))
            idx_I_l[1] +=  1. + dr.abs(dr.cos(theta))/2
            idx_I_l[0] -=  dr.sin(theta)/2
            idx_I_l = dr.floor(idx_I_l)

            mask = active & dr.all(0 <= idx_I_l) & dr.all(idx_I_l < N)
            weight_left = dr.gather(type(I), I, flat_index(idx_I_l), mask)
            center_pixel = idx_I_l + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            length_left = evaluate_spline(y)
         
            point_interest = dr.select(r.o[0] <= r_next.o[0], r.o[1], r_next.o[1])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)

            P += weight_left * length_left * checker * small_cond # Update line integral estimates

            # Compute for "LeftLeft" neighbor
            idx_I_l_l = dr.floor(0.5 * (r_next.o + r.o))
            idx_I_l_l[1] +=  2. + dr.abs(dr.cos(theta))/2
            idx_I_l_l[0] -=  dr.sin(theta)/2
            idx_I_l_l = dr.floor(idx_I_l_l)

            mask = active & dr.all(0 <= idx_I_l_l) & dr.all(idx_I_l_l < N)
            weight_left_left = dr.gather(type(I), I, flat_index(idx_I_l_l), mask)
            center_pixel = idx_I_l_l + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            length_left_left = evaluate_spline(y)

            point_interest = dr.select(r.o[0] <= r_next.o[0], r.o[1], r_next.o[1])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)
         
            P += weight_left_left * length_left_left * checker * small_cond # Update line integral estimates

            # Compute for "Right" neighbor

            idx_I_r =  dr.floor(0.5 * (r_next.o + r.o))
            idx_I_r[1] -=   dr.abs(dr.cos(theta))/2
            idx_I_r[0] +=   1 + dr.sin(theta)/2
            idx_I_r = dr.floor(idx_I_r)

            mask = active & dr.all(0 <= idx_I_r) & dr.all(idx_I_r < N)
            weight_right = dr.gather(type(I), I, flat_index(idx_I_r), mask)
            center_pixel = idx_I_r + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            ...
            P += weight_right * length_right * checker * small_cond # Update line integral estimates

            # Compute for "RightRight" neighbor
            idx_I_r_r =  dr.floor(0.5 * (r_next.o + r.o))
            idx_I_r_r[1] -=   1 + dr.abs(dr.cos(theta))/2
            idx_I_r_r[0] +=   1 + dr.sin(theta)/2
            idx_I_r_r = dr.floor(idx_I_r_r)

            mask = active & dr.all(0 <= idx_I_r_r) & dr.all(idx_I_r_r < N)
            weight_right_right = dr.gather(type(I), I, flat_index(idx_I_r_r), mask)
            center_pixel = idx_I_r_r + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)

            length_right_right = evaluate_spline(y) 

            point_interest = dr.select(r.o[0] >= r_next.o[0], r.o[1], r_next.o[1])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)
            
            P += weight_right_right * length_right_right * checker * small_cond # Update line integral estimates

            ####### LARGE COND ################
            large_cond = 1. - small_cond
                                   
            # Compute for "up" neighbor
            idx_I_u = dr.floor(0.5 * (r_next.o + r.o))
            idx_I_u[1] -= dr.sin(theta)/2 # horizontal 1 + dr.abs(dr.cos(theta))
            idx_I_u[0] += 1 + dr.cos(theta)/2 # vertical dr.sin(theta)

            idx_I_u = dr.floor(idx_I_u)

            mask = active & dr.all(0 <= idx_I_u) & dr.all(idx_I_u < N)
            weight_up = dr.gather(type(I), I, flat_index(idx_I_u), mask)
            center_pixel = idx_I_u + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            length_up = evaluate_spline(y)

            point_interest = dr.select(r.o[1] <= r_next.o[1], r.o[0], r_next.o[0])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)

            P += weight_up * length_up * checker * large_cond# Update line integral estimates

            # Compute for "upup" neighbor
            idx_I_u_u = dr.floor(0.5 * (r_next.o + r.o))
            idx_I_u_u[1] -= dr.sin(theta)/2 # horizontal 1 + dr.abs(dr.cos(theta))
            idx_I_u_u[0] += 2 + dr.cos(theta)/2 # vertical dr.sin(theta)

            idx_I_u_u = dr.floor(idx_I_u_u)

            mask = active & dr.all(0 <= idx_I_u_u) & dr.all(idx_I_u_u < N)
            weight_up_up = dr.gather(type(I), I, flat_index(idx_I_u_u), mask)
            center_pixel = idx_I_u_u + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            length_up_up = evaluate_spline(y)

            point_interest = dr.select(r.o[1] <= r_next.o[1], r.o[0], r_next.o[0])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)

            P += weight_up_up * length_up_up * checker * large_cond# Update line integral estimates

            # Compute for "down" neighbor
            idx_I_d = dr.floor(0.5 * (r_next.o + r.o))
            idx_I_d[1] += 1 + dr.sin(theta)/2 # horizontal 
            idx_I_d[0] -= dr.cos(theta)/2 # vertical 
            idx_I_d[0] -= dr.select(dr.abs(theta - dr.pi/2) < 1e-10, 1., 0.) #particular cas for pi/2
            idx_I_d = dr.floor(idx_I_d)

            mask = active & dr.all(0 <= idx_I_d) & dr.all(idx_I_d < N)
            weight_down = dr.gather(type(I), I, flat_index(idx_I_d), mask)
            center_pixel = idx_I_d + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            length_down = evaluate_spline(y)

            point_interest = dr.select(r.o[1] >= r_next.o[1], r.o[0], r_next.o[0])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)

            P += weight_down * length_down * checker * large_cond # Update line integral estimates

            # Compute for "downdown" neighbor
            idx_I_d_d = dr.floor(0.5 * (r_next.o + r.o))
            idx_I_d_d[1] += 1 + dr.sin(theta)/2 # horizontal 
            idx_I_d_d[0] -= 1 + dr.cos(theta)/2 # vertical 
            idx_I_d_d[0] -= dr.select(dr.abs(theta - dr.pi/2) < 1e-10, 1., 0.) #particular cas for pi/2
            idx_I_d_d = dr.floor(idx_I_d_d)

            mask = active & dr.all(0 <= idx_I_d_d) & dr.all(idx_I_d_d < N)
            weight_down_down = dr.gather(type(I), I, flat_index(idx_I_d_d), mask)
            center_pixel = idx_I_d_d + 0.5
            center_pixel = center_pixel - r.o
            dot = center_pixel[0]*dr.cos(theta) + center_pixel[1]*dr.sin(theta)
            y = dr.sqrt(dr.norm(center_pixel)**2 - dot**2)
            length_down_down = evaluate_spline(y)

            point_interest = dr.select(r.o[1] >= r_next.o[1], r.o[0], r_next.o[0])
            checker = dr.select(point_interest- dr.floor(point_interest) < 1e-12, 0., 1.)

            P += weight_down_down * length_down_down * checker * large_cond # Update line integral estimates
    ...
    def _xrt_apply(
        ...
        while loop(active):
            ...
            check_1 = dr.select(dr.abs(r.o[1] - idx_I_l[1]) < 1e-7, 0., 1.)
            check_2 = dr.select(dr.abs(r_next.o[1] - idx_I_l[1]) < 1e-7, 0., 1.)
            checker = check_1 * check_2

            P += weight_left * length_left * checker * small_cond # Update line integral estimates

            # Compute for "Right" neighbor

            idx_I_r =  0.5 * (r_next.o + r.o)
            idx_I_r[1] -=   1 
            idx_I_r = dr.floor(idx_I_r)
            ...
            check_1 = dr.select(dr.abs(r.o[1] - idx_I[1]) < 1e-7, 0., 1.)
            check_2 = dr.select(dr.abs(r_next.o[1] - idx_I[1]) < 1e-7, 0., 1.)
            checker = check_1 * check_2
            ...
            check_1 = dr.select(dr.abs(r.o[0] - idx_I_u[0]) < 1e-7, 0., 1.)
            check_2 = dr.select(dr.abs(r_next.o[0] - idx_I_u[0]) < 1e-7, 0., 1.)
            checker = check_1 * check_2

            P += weight_up * length_up * checker * large_cond# Update line integral estimates

            # Compute for "down" neighbor
            idx_I_d = 0.5 * (r_next.o + r.o)
            idx_I_d[0] -= 1
            idx_I_d = dr.floor(idx_I_d)
            ...
            check_1 = dr.select(dr.abs(r.o[0] - idx_I[0]) < 1e-7, 0., 1.)
            check_2 = dr.select(dr.abs(r_next.o[0] - idx_I[0]) < 1e-7, 0., 1.)
            checker = check_1 * check_2
    ...
    def finite_difference(func, h):
        ...
        """
        h = dr.select(dr.abs(h) < 1.5e-1, 1.5e-1, h) #REPLACE WITH PROPER FORMULA !! before : 1e-3 and 1e-3
        function_ =  lambda x: (func(x+h/2) - func(x - h/2)) / h
        return function_

    def radon_transform_zwart_powell_adjusted(theta, deg):
        """
        ...
        #drjit syntax
        proj = lambda x: dr.select(x >= 0, (x** (len(zetas) - 1)) , 0)/factorial(len(zetas) - 1)
    ...
    def radon_transform_zwart_powell_adjusted_(theta, deg):
        """
        Compute the Radon transform of the Zwart-Powell box spline for a given angle theta and y value.
        """
        zero_check = dr.select(dr.abs(dr.cos(theta)) < 1e-2, 0, 1) #formula changes when theta is close to zero
        pi_s_2_check = dr.select(dr.abs(dr.sin(theta)) < 1e-2, 0, 1)
        ...
        zetas = [
            ...
            dr.sin(theta),
            dr.sin(theta),
            dr.cos(theta), #stop here for pice-wise constant
            dr.cos(theta),
            dr.cos(theta),
        ]
        ...
        for zeta in zetas:
            proj_ = finite_difference(proj, zeta) 
            proj = proj_
        return proj
    ...
    def xrt_adjoint(
        ...
        while loop(active):
            ...
            # Update back-projections
            dr.scatter_reduce(dr.ReduceOp.Add, I, P *length_left * checker * small_cond, flat_index(idx_I_l), mask)
            ...
            # Update back-projections
            dr.scatter_reduce(dr.ReduceOp.Add, I, P *length_right * checker * small_cond, flat_index(idx_I_r), mask)
            ...
            # Update back-projections
            dr.scatter_reduce(dr.ReduceOp.Add, I, P *length_up * checker * large_cond, flat_index(idx_I_u), mask)
            ...
            # Update back-projections
            dr.scatter_reduce(dr.ReduceOp.Add, I, P *length_down * checker * large_cond, flat_index(idx_I_d), mask)
