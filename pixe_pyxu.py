...
def get_info(path: plib.Path):
    ...
    P = np.stack([np.array(tf.imread(f)) for f in P_files], axis=0)
    P = P.astype(np.single) / np.iinfo(P.dtype).max

    limited_angles = 1
    P = P[::limited_angles, :, :]

    return sod, sdd, v_shape, v_pitch, np.array(P)
...
def setOperator(sod, sdd, v_shape, v_pitch, P, rx_pitch, dws, roi='False', shift_cor=0, center_slice='False'):
    ...
    v_shape: tuple[int]
        (x, y, z) pixel count.
    '''
    print('sod: ', sod, 'sdd: ', sdd, 'v_shape: ', v_shape, 'v_pitch: ', v_pitch, 'rx_pitch: ', rx_pitch, 'dws: ', dws, 'roi: ', roi, 'shift_cor: ', shift_cor, 'center_slice: ', center_slice)
    v_pitch = (1, 1, 1)
    #flip image of each view
    rx_pitch = (rx_pitch[0]/0.0319955, rx_pitch[1]/0.0319955) # convert to mm
    shift_cor /= 0.0319955 # convert to mm
    sod /= 0.0319955 # convert to mm
    sdd /= 0.0319955 # convert to mm
    ...
    if center_slice == 'True':
        ...
        '''P = P.reshape(N_view*N_w, 1)  # (N_view * N_w, 1)
        N_view, N_w = P.shape''' # testing "aribitrary geometry"
    ...
    angle = -cp.linspace(0, 2 * cp.pi, num=N_view, endpoint=False) + cp.pi/573
    R = cp.zeros(
        (N_view, 3, 3),
    )
    R[:, 0, 0] = cp.cos(angle)
    ...
    # extract only the dimension 0 and 2 
    n_spec =  n_spec[:, [0, 2]]  # (N_view * N_h * N_w, 2)
    t_spec =  t_spec[:, [0, 2]]  # (N_view * N_h * N_w, 2)
    v_ll = (float(v_ll[0]),float(v_ll[2]))  # (x, z) lower-left corner of volume
    v_shape = (v_shape[0], v_shape[2])  # (x, z)
    v_pitch = (1, 1)

    print("n_spec shape: ", n_spec.shape, 
            "t_spec shape: ", t_spec.shape,
            "v_shape: ", v_shape,
            "v_pitch: ", v_pitch,
            "v_ll: ", v_ll)
...
def reconstruct(fpath, dataset, dws, rx_pitch, method = 'pinv', roi='False', shift_cor=0., center_slice='False', max_iter=40, damp=0.1):
    ...
    print("projections shape: ", P.shape)
        ...
        '''        
            ...
            class TVFunc(ProxFunc):
                ...
                def __init__(self,
                    ...
                    finite_diff_op = Gradient(arg_shape, sampling=[1, 1, 1], gpu=True)#, gpu=True) #GPU=True if gpu is available /!\
                    
                    #finite_diff_op.estimate_lipschitz()
                    finite_diff_op.lipschitz= 1430.6675#finite_diff_op.estimate_lipschitz()
                ...
                def prox(self, arr, tau = 0.05):
                    ...
                    ls.diff_lipschitz = 639.9992 #ls.estimate_diff_lipschitz() #1.0

                    slv = CV(f=ls, h=tau * self._norm, K=self._finite_diff_op,  verbosity=500)
                    slv.fit(x0=cp.array(arr.copy()), stop_crit = pxst.MaxIter(100))
            ...
            stop_crit = pxst.AbsError(eps=1e-3, var="x", f=smooth_posterior, norm=2, satisfy_all=True) | pxst.MaxIter(max_iter)
    ...
    return V_bw.reshape(v_shape), P, op
...
if __name__ == "__main__":
    ...
    # Reconstruct. ============================================================
    rec = reconstruct(fpath, dataset, dws, rx_pitch, method = 'tv', roi='False', shift_cor=shiftcorr_value, center_slice='True', max_iter=3000, damp=0.01) #200 it, damp=2 for pinv | for tv damp=damp=0.001
    
    # Plot. ===================================================================
    plt.figure()
    plt.imshow(rec.get()[200:400, rec.shape[1]//2, 200:400], cmap='gray')
    plt.show()
